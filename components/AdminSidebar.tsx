  ...
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: '12px 10px', overflowY: 'auto' }}>
        <div style={{ fontSize: '10px', fontWeight: '600', color: '#9ca3af', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0 10px', marginBottom: '6px' }}>
          Menu
        </div>
        {NAV.map(item => {
          ...
          return (
            ...
            >
              <span style={{ color: active ? '#2d6a4f' : '#9ca3af', display: 'flex', alignItems: 'center' }}>
                {item.icon}
              </span>
              {item.label}
              {active && (
                <span style={{ marginLeft: 'auto', width: '5px', height: '5px', borderRadius: '50%', background: '#2d6a4f' }} />
              )}
            </a>
          )
        })}
