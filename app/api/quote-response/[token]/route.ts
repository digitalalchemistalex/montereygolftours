...
const LOGO = `<table cellpadding="0" cellspacing="0" border="0" width="100%"><tr><td style="vertical-align:middle;padding-right:16px;width:1%"><a href="https://montereygolftours.com" style="display:block;text-decoration:none"><img src="https://montereygolftours.vercel.app/brand/logo-transparent-master.png" alt="Monterey Golf Tours" width="72" height="72" style="display:block;width:72px;height:72px;object-fit:contain;border-radius:50%;border:0" /></a></td><td style="vertical-align:middle"><div style="color:#fff;font-size:18px;font-weight:800;letter-spacing:-0.2px;line-height:1.2">Monterey Golf Tours</div><div style="color:#C9A24D;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;margin-top:3px">Pebble Beach &middot; Carmel &middot; Monterey Peninsula</div></td></tr></table>`;

const EMAIL_HEADER = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;padding:0;background:#F3F4F6;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;}.wrap{max-width:600px;margin:24px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);}table{border-collapse:collapse;}</style></head><body><div class="wrap"><div style="background:#1E3A2F;padding:16px 28px">${LOGO}</div><div style="height:3px;background:linear-gradient(90deg,#C9A24D,#B08C3A)"></div>`;

const EMAIL_FOOTER = `<div style="padding:16px 28px;font-size:11px;color:#9CA3AF;text-align:center;border-top:1px solid #E5E7EB">Monterey Golf Tours &nbsp;&middot;&nbsp; 2700 Mill St Suite 800, Reno, NV 89502<br><a href="mailto:info@montereygolftours.com" style="color:#9CA3AF">info@montereygolftours.com</a></div></div></body></html>`;

async function sendNotification(subject: string, body: string) {
  const key = process.env.RESEND_API_KEY;
  if (!key) return;
  const html = EMAIL_HEADER + '<div style="padding:28px 32px">' + body + '</div>' + EMAIL_FOOTER;
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const body = '<h2 style="margin:0 0 16px;font-size:22px;font-weight:800;color:#111">Quote ' + statusLabel + '</h2>'
