...
export default async function AdminDashboard() {
  ...
  const navCards = [
    { href: '/admin/leads',  label: 'Lead Pipeline', desc: 'View, score, and act on every quote request', bg: '#f0fdf4', border: '#bbf7d0', color: '#2d6a4f' },
    { href: '/admin/quotes', label: 'Quote Builder', desc: 'Build custom trip quotes and send to clients',  bg: '#eff6ff', border: '#bfdbfe', color: '#1d4ed8' },
    { href: '/admin/rates',  label: 'Rate Configs',  desc: 'Manage course and hotel net rates by season',  bg: '#fffbeb', border: '#fde68a', color: '#92400e' },
    { href: '/admin/health', label: 'System Health', desc: 'Env vars, deployments, and live site checks',  bg: '#faf5ff', border: '#e9d5ff', color: '#6d28d9' },
  ]
  ...
  return (
    <div style={{ padding: '40px 48px', fontFamily: '-apple-system, BlinkMacSystemFont, "Inter", sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px' }}>
        {navCards.map(c => (
          <a key={c.href} href={c.href} style={{ display: 'block', background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '22px 24px', textDecoration: 'none', borderLeft: '3px solid ' + c.color }}>
